...
> 운영 로그의 장비 상태 변화와 업무 처리 결과를 분석하는 웹 서비스

## 현재 1차 분석 기능

- 장비 상태 덤프·통신 로그: 상품명/슬롯/비정상 문자 검사, 가격·상품명·재고 변화, 슬롯 재등장.
- 외부 시스템 재전송 로그: 명시적 SQL 성공, 실패, 결과 미확인과 다중 줄 근거.
- 외부 연동 데이터 로그: 기존 요청·응답 결과 분석 및 중복 응답 근거 병합.
- 장비·슬롯·업무일·전후 근거 저장, 분석 범위 표시, 규칙 스냅샷과 저장 롤백.

검증 결과, 결과 계약, DB 마이그레이션 및 후속 범위는 [1차 구현·검증 문서](docs/phase1-validation.md)를 참고하세요. 기존 DB에는 애플리케이션 배포 전에 [증분 변경](db/migrations/20260917_phase1_analysis.sql)을 적용해야 합니다. 실제 로그 원문은 저장소에 포함하지 않습니다.

아래는 프로젝트의 배경과 초기 계획을 포함한 소개입니다. 현재 구현 범위와 데이터 구조는 위 검증 문서와 코드가 기준입니다.
